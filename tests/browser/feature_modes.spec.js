...
test("Radar target workflow, monostatic payload and result selections stay linked", async ({
  page,
}) => {
  ...
  expect(
    await radarGroups.evaluateAll((groups) =>
      groups.map((group) => group.open),
    ),
  ...
  const assetPickerStyles = await page.evaluate(() => {
    const picker = getComputedStyle(
      document.getElementById("radarAssetPicker"),
    );
    const viewport = getComputedStyle(
      document.querySelector(".radarAssetPreviewViewport"),
    );
    const nav = getComputedStyle(
      document.getElementById("btnRadarAssetPrevious"),
    );
    const count = getComputedStyle(
      document.getElementById("radarAssetPreviewCount"),
    );
    const addButton = getComputedStyle(
      document.getElementById("btnAddRadarTarget"),
    );
    return {
  ...
  const targetActionLayout = await page.evaluate(() => {
    ...
    const buttonWidths = [...actionBar.querySelectorAll("button")].map(
    ...
    return {
      ...
      precedesTargetList: Boolean(
        actionBar.compareDocumentPosition(
          document.getElementById("radarTargetList"),
        ) & Node.DOCUMENT_POSITION_FOLLOWING,
      ),
      buttonWidths,
      columns:
        getComputedStyle(actionBar).gridTemplateColumns.split(" ").length,
    };
  });
  ...
  expect(targetActionLayout.actionTop).toBeGreaterThan(
    targetActionLayout.addButtonBottom,
  );
  expect(
    Math.abs(
      targetActionLayout.actionWidth - targetActionLayout.addButtonWidth,
    ),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.max(...targetActionLayout.buttonWidths) -
      Math.min(...targetActionLayout.buttonWidths),
  ).toBeLessThanOrEqual(1);
  await expect(page.locator(".radarEditorActions")).toHaveAttribute(
    "aria-label",
  ...
  expect(
    await page
      .locator(".radarEditorActions button")
  ...
  await expect(page.locator("#radarAssetPreviewCanvas")).toHaveAttribute(
    "aria-label",
    "Interactive 3D preview of DJI Air 2S",
  );
  await page.locator("#btnRadarAssetPrevious").click();
  await expect(page.locator("#radarAssetPreviewName")).toHaveText(
    "DJI Mini 3 Pro",
  );
  ...
  const collapsedRadarGroupLayout = await page
    .locator("#radarPanel .radarGroup")
    .first()
    .evaluate((group) => {
      group.open = false;
      ...
      return {
        ...
        centerOffset: Math.abs(
          summaryRect.top +
            summaryRect.height / 2 -
            (groupRect.top + groupRect.height / 2),
        ),
      };
    });
  const sharedSummaryHeight = await page
    .locator("#modeSelector > summary")
  ...
  await expect(page.locator("#radarEditorTitle")).toHaveText(
    "No target selected",
  );
  await expect(page.locator("#radarTargetX")).toHaveValue("");
  await expect(page.locator("#btnSolveRadar")).toBeDisabled();
  await expect(page.locator("#btnSolveRadar")).toHaveAttribute(
    "title",
    "Place Radar Tx and Rx before running sensing.",
  );
  await expect(page.locator("#btnOrbitTx")).toBeDisabled();
  ...
  await expect(page.locator("#radarVelocityVectorPreview")).toHaveText(
    "Velocity [0.0, 7.5, 0.0] m/s",
  );
  expect(
    await page.evaluate(async () => {
      const { state } =
  ...
  await page.evaluate(async () => {
    ...
    Object.assign(state.radar.targets[1], {
      position: [126, 50, 54],
      orientation: [0, 0, -2.478368],
      velocity: [-10.24414, -8.003599, 0],
      rcs_m2: 0.018,
  ...
  const equivalentDopplers = configured.targets.map((target) => {
    const unit = (origin) => {
      const delta = target.position.map(
        (value, index) => value - origin[index],
      );
      const length = Math.hypot(...delta);
      return delta.map((value) => value / length);
    };
    const txUnit = unit(configured.tx);
    const rxUnit = unit(configured.rx);
    const pathRate = target.velocity.reduce(
      (sum, value, index) => sum + value * (txUnit[index] + rxUnit[index]),
      0,
    );
    return -pathRate / (299792458 / 5.8e9);
  });
  expect(Math.sign(equivalentDopplers[0])).toBe(
    -Math.sign(equivalentDopplers[1]),
  );
  ...
  const initialDirectionArrows = await page.evaluate(async () => {
    const { viewerRef } =
      await import("/js/app_state.js?v=20260723-radar-shared-groups");
    return viewerRef.current.layers
  ...
  await expect
    .poll(() =>
      pickTargetButton.evaluate(
        (button) => getComputedStyle(button).backgroundColor,
      ),
    )
    .toBe("rgb(31, 111, 255)");
  expect(
    await pickTargetButton.evaluate(
  ...
  await page.evaluate(async () => {
    ...
    viewer.camera.position.set(
      selected.position[0] - 28,
      selected.position[1] - 36,
      selected.position[2] + 22,
    );
    viewer.camera.lookAt(viewer.controls.target);
    viewer.controls.update();
  });
  ...
  expect(crowdedLayout.labels.map(({ id }) => id).sort()).toEqual([
    "target-1",
    "target-2",
  ]);
  expect(
    crowdedLayout.labels.every(
      ({ anchorX, rect }) => rect.left >= anchorX + 11,
    ),
  ).toBe(true);
  ...
  await expect(
    page.locator("#radarTargetVx, #radarTargetVy, #radarTargetVz"),
  ).toHaveCount(0);
  ...
  for (const [enteredDirection, expectedDirection] of [
    ["0", 0],
    ["90", 90],
    ["-142", -142],
    ["218", -142],
    ...
    const heading = await page.evaluate(async () => {
      ...
      return {
        directionDeg:
          (Math.atan2(target.velocity[1], target.velocity[0]) * 180) / Math.PI,
  ...
  const labelPresentation = await targetLabel.evaluate((node) => {
    ...
    const resultStyle = getComputedStyle(
      document.getElementById("linkChannelSection"),
    );
    ...
    return {
      ...
      parentIsShell:
        node.parentElement.parentElement.classList.contains("shell"),
      layerIsFirstChild:
        node.parentElement.parentElement.firstElementChild ===
        node.parentElement,
    };
  });
  expect(labelPresentation).toMatchObject({
    pointerEvents: "none",
    layerPosition: "fixed",
    parentIsShell: true,
    layerIsFirstChild: true,
  });
  ...
  expect(
    Math.abs(farLabelMetrics.width - nearLabelMetrics.width * 0.6),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.abs(farLabelMetrics.height - nearLabelMetrics.height * 0.6),
  ).toBeLessThanOrEqual(1);
  ...
  await page.evaluate(async () => {
    ...
    const viewer = viewerRef.current;
    const underControls = new THREE.Vector3(-0.75, 0, 0).unproject(
      viewer.camera,
    );
  ...
  await expect(
    page.locator("#devicePrecisionPanel .deviceCoordPanel:not(.hidden)"),
  ).toHaveCount(1);
  const precisionBounds = await page
    .locator("#devicePrecisionPanel")
    .boundingBox();
  ...
  await expect(page.locator("#radarRangeDopplerCanvas")).toHaveCSS(
    "background-color",
    "rgb(247, 249, 252)",
  );
  await expect(page.locator("#radarRdExpand")).toHaveCount(0);
  await expect(
    page.locator("#radarPanel").getByText("Live Preview", { exact: true }),
  ).toHaveCount(0);
  expect(submittedRadar.mode).toBe("monostatic");
  expect(submittedRadar.solver).toMatchObject({
    samples_per_src: 65536,
    diffuse_reflection: true,
  });
  expect(submittedRadar.rx.position).toEqual(submittedRadar.tx.position);
  expect(submittedRadar.targets[0]).toMatchObject({
    position: [96, 44, 30],
    rcs_m2: 0.025,
  });
  expect(submittedRadar.targets[0].orientation[0]).toBeCloseTo(
    (12 * Math.PI) / 180,
    6,
  );
  expect(submittedRadar.targets[0].orientation[1]).toBeCloseTo(
    (-7 * Math.PI) / 180,
    6,
  );
  ...
  expect(linkage).toMatchObject({
    target: submittedRadar.targets[0].id,
    detection: "det-0",
    selectedPath: 0,
    sharedPaths: 0,
    radarPaths: 2,
  ...
  expect(linkage.targetOverlays[0].data).toMatchObject({
    targetId: submittedRadar.targets[0].id,
    speedMps: 12,
    rcsM2: 0.025,
  });
  expect(linkage.targetOverlays[0].names).toEqual([
    `radar-target-velocity-${submittedRadar.targets[0].id}`,
  ]);
  const targetColorContract = await page.evaluate(async () => {
    ...
    const clutterLegendStyle = getComputedStyle(
      legend.querySelector(".clutter"),
      "::before",
    );
    const result = {
      expectedColor,
  ...
  await expect(
    page.locator(
      `.radarTargetLabel[data-target-id="${submittedRadar.targets[0].id}"]`,
    ),
  ...
  expect(
    await page.locator("#radarPanel, #radarResultSections").allTextContents(),
  ).not.toEqual(
    expect.arrayContaining([expect.stringMatching(/download|export/i)]),
  );
  await page.setViewportSize({ width: 1280, height: 720 });
  await expect(page.locator("#radarPanel")).toBeVisible();
  expect(
    await page
      .locator(".radarVectorGrid")
      .first()
      .evaluate(
  ...
  expect(
    (await page.locator("#radarRangeDopplerCanvas").boundingBox()).width,
  ).toBeGreaterThan(300);
