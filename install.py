...
POWERSHELL_ENV_FILE = PROJECT_ROOT / ".oat-env.ps1"
...
def format_powershell_env_file(values: dict[str, str]) -> str:
    lines = [
        "# OpenAirTwin local runtime environment for PowerShell.",
        "# Generated by install.py. This file is intentionally not committed.",
    ]
    for key in sorted(values):
        lines.append(f"$env:{key} = {powershell_quote(values[key])}")
    return "\n".join(lines) + "\n"
...
def write_powershell_env_file(
    values: dict[str, str],
    path: Path = POWERSHELL_ENV_FILE,
    *,
    dry_run: bool = False,
) -> None:
    if dry_run:
        print(f"[dry-run] Would write {path}")
        return
    path.write_text(format_powershell_env_file(values), encoding="utf-8")
    print(f"Wrote PowerShell environment config: {path}")
...
def launch_command(system_name: str | None = None) -> str:
    py = venv_python_path()
    if platform_key(system_name) == "windows":
        return ". .\\.oat-env.ps1; .\\.venv\\Scripts\\python.exe -m backend.server"
...
def run_install(args: argparse.Namespace) -> int:
    ...
    if platform_key() == "windows":
        write_powershell_env_file(env_values, POWERSHELL_ENV_FILE, dry_run=args.dry_run)
